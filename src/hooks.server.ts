    ...
    if (dashboard || path.startsWith("/auth")) {
        ...
        if (dashboard) {
            ...
            if (!event.locals.session)
                throw redirect(303, "/auth/login?redirect=" + encodeURI(path));
        }
        ...
        const sb = createSupabaseServerClient({
            ...
            supabaseKey: PUBLIC_SUPABASE_KEY,
            event,
            cookieOptions: BUILD_cookieOptions,
        });
        event.locals.supabase = sb;
        event.locals.session = null;
        ...
        if (!session) {
            const res = await sb.auth.signInWithPassword({
                email: SUPABASE_READER_EMAIL,
                password: SUPABASE_READER_PSW,
            });
            if (res.error) console.log("Could not login reader");
            else console.log("Reader login succesful");
        }
        return resolve(event, {
            filterSerializedResponseHeaders(name) {
                return name === "content-range";
