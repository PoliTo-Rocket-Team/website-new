<script lang="ts">
    ...
    const decimals = numeric
        ? Math.max.apply(Math, field.values.map(countDecimals))
        : 0;
    ...
    function onvisible(visible: boolean) {
        ...
        unsub && unsub();
        if (visible)
            unsub = selection.subscribe(i => smooth!.set(field.values[i]));
        else {
            smooth!.set(0);
            unsub = null;
        }
...
<!-- svelte-ignore -->
<span class="value"
    >{#if numeric}{$smooth.toFixed(decimals)}{:else}{field.values[
            $selection
        ]}{/if}
    {#if field.unit}{field.unit}{/if}</span
>
