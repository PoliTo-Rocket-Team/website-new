...
function setSceneBg(dark: boolean) {
    scene.background = new Color(dark ? 0x121212 : 0xefefef);
}
...
directional_light.position.set(2, 0.5, 1);
scene.add(directional_light);

const ambient_light = new AmbientLight(0xffffff, 0.2);
scene.add(ambient_light);

function randomDisplacement(w: number = 3e-3) {
...
const post = getCanvasWorker<WorkerMessageMap, HostMessageMap>(
    data => {
...
function startAnimation(obj: Object3D) {
    ...
    function animate(time: number) {
        ...
        if (height < liftoff_heigth) {
            height +=
                liftoff_coefficient * Math.pow(liftoff_heigth - height, 1.5);
            obj.position.z = height;
        }
        camera.position.set(1, 0, liftoff_heigth + 6);
        camera.position.add(
            new Vector3(0, yshift, camera_distance).applyEuler(rot)
        );
...
function addAxisArrows(origin?: Vector3) {
    ...
    scene.add(
        new ArrowHelper(new Vector3(1, 0, 0), origin, 1, 0xff0000, 0.2, 0.1),
        new ArrowHelper(new Vector3(0, 1, 0), origin, 1, 0x00ff00, 0.2, 0.1),
        new ArrowHelper(new Vector3(0, 0, 1), origin, 1, 0x0000ff, 0.2, 0.1)
    );
}
...
export async function loadScene() {
    ...
    try {
        ...
        const gltf = await loader.loadAsync("/assets/Rocket.glb", e =>
            post("load", e.loaded / e.total)
        );
