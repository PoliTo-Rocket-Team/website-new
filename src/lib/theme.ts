...
if(!browser) {
    ...
    theme_media.addEventListener("change", e => system.set(e.matches ? "dark" : "light"));
    preference = writable(localStorage.getItem(RECORD) as ThemePreference || "system");
    preference.subscribe(v => localStorage.setItem(RECORD, v)); 
}

export { system, preference };

export const theme = derived([preference, system], ([p,s]) => p === "system" ? s : p);
