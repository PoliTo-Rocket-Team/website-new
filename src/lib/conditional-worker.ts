...
export type CanvasPoster<M extends CanvasMessageMap> = <T extends keyof M>(type: T, data: M[T], transfer?: Transferable[]) => void;
type ReceiveCallbacks<M extends CanvasMessageMap> = { [T in keyof M]?: (data: M[T]) => void; }
interface CanvasConnection<W extends CanvasMessageMap, H extends CanvasMessageMap> {
...
export interface CanvasData<O extends boolean> {
    canvas: O extends true ? HTMLCanvasElement|OffscreenCanvas : HTMLCanvasElement;
    width: number;
    height: number;
    pixelRatio: number;
}

export function alleviateCanvas<Send extends CanvasMessageMap, Receive extends CanvasMessageMap = CanvasMessageMap>(
    ...
    if(data instanceof HTMLElement) {
        const newData: CanvasData<false> = {
            canvas: data, pixelRatio: window.devicePixelRatio,
            width: data.clientWidth, height: data.clientHeight,
        };
        data = newData;
    }
    ...
    if(supported) {
        try {
            ...
            if(listeners) worker.onmessage = (ev) => {
                const [type, data] = ev.data;
                const fn = listeners[type];
                fn && fn(data);
            }
            //@ts-ignore
            return (type, data, transfer) => worker.postMessage([type,data], transfer);
        }
        catch {
            console.warn(`Could not load worker at ${typeof url === "string" ? url : url.href}`);
        }
    }
    const id = "worker-"+Math.random().toString(36).slice(2);
    const script = document.createElement("script");
    ...
    const connection: CanvasConnection<Send, Receive> = {
        msgs: [],
        toWorker(type, data) { this.msgs.push([type, data]); },
        toHost: listeners && ((type, data) => {
            const fn = listeners[type];
            fn && fn(data);
        }),
    }
...
export function getCanvasWorker<Send extends CanvasMessageMap, Receive extends CanvasMessageMap = CanvasMessageMap>(
